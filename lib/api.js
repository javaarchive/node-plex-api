...
PlexAPI.prototype._request = function _request(options) {
    ...
    return new Promise((resolve, reject) => {
        ...
        fetch(reqUrl,reqOpts).then(onResponse).catch(reject);
    });
};
...
PlexAPI.prototype._defaultResponseParser = function _defaultResponseParser(response, body) {
    ...
    } else if (!response.headers['content-type'] || response.headers['content-type'].startsWith("audio") || response.headers['content-type'].startsWith("video") || response.headers['content-type'].startsWith("application/octet-stream")) {
