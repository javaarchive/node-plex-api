...
function PlexAPI(options, deprecatedPort) {
    ...
    this.hostname = hostname || "127.0.0.1"; // localhost default
...
PlexAPI.prototype._request = function _request(options) {
    ...
    return new Promise((resolve, reject) => {
        let onResponse = function (response, body) {
        ...
        fetch(reqUrl,reqOpts).then(async (resp) => {
            let body = await resp.buffer();
            let headerObj = {};
            resp.headers.forEach((value, key) => {
                headerObj[key] = value;
                if(value && value.length && typeof value != 'string'){
                    headerObj[key] = value[0];
                }
            });
            resp.headersMap = headerObj;
...
PlexAPI.prototype._defaultResponseParser = function _defaultResponseParser(response, body) {
    // console.log("parsing", body.toString("utf8"));
    // console.log(response.headersMap);


    if (response.headersMap['content-type'] === 'application/json') {
        return Promise.resolve(JSON.parse(body.toString('utf8')))
    }else if (response.headersMap['content-type'] && response.headersMap['content-type'].indexOf('xml') > -1) {
        return xmlToJSON(body.toString('utf8'), {
            attrkey: 'attributes',
        });
    } else if (!response.headersMap['content-type'] || response.headersMap['content-type'].startsWith("audio") || response.headersMap['content-type'].startsWith("video") || response.headersMap['content-type'].startsWith("application/octet-stream")) {
        return Promise.resolve(body.toString('utf8'));
    }
    console.warn("Unhandled Response Parsing Case");
